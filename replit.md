...
3. **Team Management** — `/dashboard/team` (admin only): list members, change roles, remove with confirm dialog, invite by email (generates invite link + "Enviar por correo" mailto: button), pending invites list. Invite acceptance page at `/invite/:token`. **NOTE: Real email sending requires `RESEND_API_KEY` secret — not yet configured (user dismissed Resend integration). Current behavior: admin clicks "Enviar por correo" which opens their email client via mailto: with pre-filled message. To add real email, set `RESEND_API_KEY` and implement in `server/email.ts` using `@resend/node` package.**
